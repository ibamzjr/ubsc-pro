            ...
            <div className="pt-6 pb-20 flex flex-col gap-5">
                ...
                <div className="relative card-glint shimmer-once overflow-hidden flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-2xl border border-slate-200/80 bg-white px-5 py-4 shadow-[0_1px_4px_rgba(0,0,0,0.06)] animate-fade-in-up delay-100">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="font-clash text-xs font-medium text-slate-400 leading-snug mt-0.5 max-w-sm">
                                Seret kartu untuk mengatur urutan tampil di marquee.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2.5 sm:shrink-0">
                        ...
                        <button
                            type="button"
                            onClick={openNew}
                            className="btn-sheen relative flex items-center gap-2 rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-500 px-4 py-2.5 text-sm font-medium text-white shadow-[0_4px_14px_rgba(15,23,42,0.25),inset_0_1px_0_rgba(255,255,255,0.1)] transition-all duration-200 hover:shadow-[0_6px_20px_rgba(15,23,42,0.3)] hover:-translate-y-0.5 active:translate-y-0"
                        >
                            <span className="pointer-events-none absolute top-0 left-0 right-0 h-px rounded-t-2xl bg-white/20" />
                            <Plus size={15} />
                            Tambah Sponsor
                ...
                {items.length === 0 ? (
                    <div className="animate-fade-in-up delay-200 flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-slate-200 bg-white py-16 text-center">
                        ...
                        <p className="font-bdo text-sm text-slate-400">Belum ada sponsor. Tambahkan yang pertama!</p>
                    </div>
                ) : (
                    <DndContext
                        sensors={sensors}
                    ...
                    >
                        ...
                        >
                            <div className="animate-fade-in-up delay-200 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                                {items.map((item, idx) => (
                                    <SortableCard
                                        key={item.id}
                                        id={item.id.toString()}
                                    >
                                        {(handle) => (
                                            <div className={cn(
                                                ...
                                                {/* Logo thumbnail */}
                                                <div className="relative flex aspect-square w-full items-center justify-center overflow-hidden bg-slate-50 p-4">
                                                    {item.logo_url ? (
                                                        <img
                                                            src={item.logo_url}
                                                            alt={item.name}
                                                        ...
                                                        <div className="flex flex-col items-center gap-1 text-slate-300">
                                                            <ImageIcon size={24} />
                                                            <span className="font-bdo text-[9px]">No Logo</span>
                                                        </div>
                                                    )}
                                                    ...
                                                    <div className="absolute top-2 right-2">
                                                        <span className="rounded-full bg-black/30 px-1.5 py-0.5 font-bdo text-[10px] font-bold text-white/80 backdrop-blur-sm">
                                                            #{idx + 1}
                                                        </span>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex flex-col gap-1.5 p-2.5">
                                                    <div className="flex items-center justify-between gap-1 min-w-0">
                                                        <p className="font-clash text-xs font-semibold text-slate-800 truncate">
                                                            {item.name}
                                                        </p>
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        <button
                                                            ...
                                                            className="flex flex-1 items-center justify-center gap-1 rounded-xl border border-slate-200 bg-slate-50 py-1 text-[10px] font-bdo text-slate-600 transition-all hover:bg-slate-100"
                                                        >
                                                            <Pencil size={9} /> Edit
                                                        </button>
                                                        <button
                                                            ...
                                                            className="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl border border-rose-100 bg-rose-50 text-rose-400 transition-all hover:bg-rose-100"
                                                        >
                                                            <Trash2 size={10} />
                                                        </button>
                                                    </div>
